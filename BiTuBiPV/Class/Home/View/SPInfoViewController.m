...
-(void)_loadWebView{
    ...
    NSURLRequest *request = [[NSURLRequest alloc]initWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@&phone=%@",BTBUrl,self.Url,strData,umodel.UserName]]];
//    NSLog(@"%@",self.Url);
//    NSLog(@"%@",request.URL);
...
//            NSLog(@"加入购物车");
    ...
    if ([request.URL.absoluteString hasPrefix:@"jsoc6:"]) {
        ...
        NSArray *arrData = [strUrl componentsSeparatedByString:@":/"];
//        NSLog(@"%@",arrData[1]);
        NSString *myurl = arrData[1];
        
        //1.创建 请求操作的管家对象
        AFHTTPRequestOperationManager * manager = [AFHTTPRequestOperationManager manager];
        
        //2.url(不要拼接)
        NSString * string = [NSString stringWithFormat:@"%@%@",BTBUrl,myurl];
...
//            NSLog(@"成功");
    ...
    if ([request.URL.absoluteString hasPrefix:@"jsoc7:"]) {
        NSString *strUrl = [NSString stringWithFormat:@"%@",request.URL];
        strUrl = [strUrl substringFromIndex:6];
        strUrl = [self URLDecodedString:strUrl];
        NSDictionary *data = [self Jx:strUrl];
//        NSLog(@"%@",data);
    ...
    if([request.URL.absoluteString hasPrefix:@"jsoc10:"]){
//        NSLog(@"添加并跳转");
        NSString *strUrl = [NSString stringWithFormat:@"%@",request.URL];
