...
@interface ShopFLViewController ()<UITextViewDelegate>
{
    UIWebView *_webView;
...
@implementation ShopFLViewController
...
- (void)viewDidLoad {
    ...
    _txtView.layer.cornerRadius = 10.0;
    _txtbut.backgroundColor = BTBColor(248, 248, 248);
    _txtView.layer.borderColor = BTBColor(204, 204, 204).CGColor;
    _txtView.layer.borderWidth = 0.5;
    _txtView.delegate = self;
    [_txtView addSubview:_txtbut];
...
-(void)_loadWebView{
    ...
    NSURLRequest *request = [[NSURLRequest alloc]initWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@",BTBUrl,self.Url,strData]]];
    NSLog(@"%@",request.URL);
...
-(void)butAction:(UIButton *)button{
    ...
    NSString *str = [NSString stringWithFormat:@"%@",[self URLEncodedString:self.txtView.text]];
    self.Url = [NSString stringWithFormat:@"%@%@",BTB_Select_list,str];
    [self _loadWebView];
